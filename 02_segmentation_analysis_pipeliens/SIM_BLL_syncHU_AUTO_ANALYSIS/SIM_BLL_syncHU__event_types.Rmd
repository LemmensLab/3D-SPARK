---
title: "SIM_BLL_syncHU__event_types"
---
...
paths <- Sys.glob("*ss_1*/*/*/overlap_tables/two_channel_initiation__647_dye_vs_555_dye_105_overlap_labels.csv.gz")
Init <- map(paths, fread, header = TRUE, sep = '\t')
names(Init) = paths
L$initiation <- rep(0,dim(L)[1])
for(file in names(Init)){
  L$initiation[which(L$path == gsub("overlap_tables/two_channel_initiation__647_dye_vs_555_dye_105_overlap_labels.csv.gz","",file))] = dim(Init[[file]])[1]
  }

paths <- Sys.glob("*ss_1*/*/*/overlap_tables/two_channel_ongoing__647_dye_vs_555_dye_105_overlap_labels.csv.gz")
Ong <- map(paths, fread, header = TRUE, sep = '\t')
names(Ong) = paths
L$ongoing <- rep(0,dim(L)[1])
for(file in names(Ong)){
  L$ongoing[which(L$path == gsub("overlap_tables/two_channel_ongoing__647_dye_vs_555_dye_105_overlap_labels.csv.gz","",file))] = dim(Ong[[file]])[1]
  }

paths <- Sys.glob("*ss_1*/*/*/overlap_tables/two_channel_termination__647_dye_vs_555_dye_105_overlap_labels.csv.gz")
Term <- map(paths, fread, header = TRUE, sep = '\t')
names(Term) = paths
L$termination <- rep(0,dim(L)[1])
for(file in names(Term)){
  L$termination[which(L$path == gsub("overlap_tables/two_channel_termination__647_dye_vs_555_dye_105_overlap_labels.csv.gz","",file))] = dim(Term[[file]])[1]
  }
